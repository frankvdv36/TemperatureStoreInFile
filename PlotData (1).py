...
# naming the x-axis
plt.xlabel('aantal metingen (iedere 5min)')              # tekst x-as
...
ax.spines['left'].set_bounds(-10, 60)    # lengte lijn y-as
  
# set the interval by  which 
# the x-axis set the marks
plt.xticks(list(range(0, lengte+12, 12)))         # X-as van -3 tot 9 (laatste element niet)
  
# set the intervals by which y-axis
# set the marks
plt.yticks(list(range(-10, 65, 5)))      # Y-as van -3 tot 18 (laatste element niet)
...
plt.annotate('1 STREEPJE = 1 UUR met 12 metingen', xy = (0, -9))  # tekst boven x-as extra uitleg + coordinaten (1.01, -2.15)
  
# gives a title to the Graph
plt.title('CV temperatuurverloop')             # titel bovenaan
