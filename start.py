...
#import board                           # mogelijk niet van doen
...
def read_timedate():
    ...
    now = datetime.now()		# Voorbeeld	 #   22-09-26 18:00:21.994273
    tijd = now.strftime("%H:%M:%S")          #   18:00:21 na filteren
    datum = date.today()					 #   2022-09-26
...
def write_file(filedata): # schrijf de meegegeven data op SD kaart
    ...
    fo = open("/home/pi/Python3/SD18B20/data.txt", "a") # open file volgens pad, indien onbestaand maak aan
...
# Rechts onder Temp H-L
    ...
    def draw4(self, percent):
        ...
        tempD = tempH - tempL 
        fill_angle = int((tempD)*270/50)       # hoek per 50%  vb (270* 1000/500)
        ...
        self.screen.blit(self.font.render('0', True, ac4), (int(self.x_cord*0.9), int(self.y_cord*1.14)))              # cijfer begin boog
        self.screen.blit(self.font.render('50', True, ac4), (int(self.x_cord*1.05), int(self.y_cord*1.14)))              # cijfer einde boog
        self.screen.blit(pertext, pertext_rect) # laat text zijn binnen boog (zonder deze lijn zien we enkel de boog)
...
while True:     # Routine voor sensoren SD18B20 + Datafile invullen
    ...
    try:
        ...
        tempB = round (temp,1)      # Sensor met gele band ..5A8F
